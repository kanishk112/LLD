...
public class Main {
    public static void main(String args[]) {
        ...
        ConcertTicketBookingService bookingSystem = ConcertTicketBookingService.getInstance();
    ...
    private static List<Seat> generateSeats(int numberOfSeats) {
        ...
        for (int i = 1; i <= numberOfSeats; i++) {
            ...
            SeatClass seatClass = (i <= 10) ? SeatClass.VIP : (i <= 30) ? SeatClass.PREMIUM : SeatClass.REGULAR;
            double price = (seatClass == SeatClass.VIP) ? 100.0 : (seatClass == SeatClass.PREMIUM) ? 75.0 : 50.0;
            seats.add(new Seat(seatNumber, seatNumber, seatClass, price));
        }
        return seats;
    }
